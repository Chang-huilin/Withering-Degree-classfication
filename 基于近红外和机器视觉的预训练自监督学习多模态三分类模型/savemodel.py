...
def load_model():
    ...
    # TODO: 改成你的权重路径与格式
    # 常见：state_dict = torch.load("xxx.pth", map_location="cpu")
    #      model.load_state_dict(state_dict)
    # 如果你保存的是整个模型：model = torch.load("xxx.pth", map_location="cpu")
    # 你自己按实际选择一种
...
def main():
    ...
    print("nir shape:", tuple(nir.shape))
    print("out shape:", tuple(y.shape))
    print("out sample:", y[0].cpu().numpy())
